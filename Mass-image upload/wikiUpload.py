...
class wikiUploader:
	...
	def exists(self, destfile):
		return self.buildGrab('File:' + (destfile[0].upper() + destfile[1:]).replace(' ', '_')).find('No file by this name exists') == -1 # Hack
	def upload(self, filename, destfile, pagecontent='', license='', overwrite=False, reupload=False):
		...
		editTokenContainer = self.buildGrab('Special:Upload')
		editTokenMatch = re.search("""<input[^<>]*(?:id|name)=['"]?wpEditToken['"]?[^<>]*value=['"]?([^"]+)['"]?""", editTokenContainer, re.IGNORECASE)
		self.editToken = urllib.unquote(editTokenMatch.group(1))
